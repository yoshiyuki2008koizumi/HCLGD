...
function handleCellAction(add, td, event) {  // (左ダブルクリック-false、右クリック=true
 //console.log("handleCellAction fired", performance.now());
  ...
  if (ds.col === "0"){  //col==0なら差分値の設定
    ...
    if(name.endsWith("_i")){ //差分変数
      ...
      const ok = confirm(`${name} の差分値を ${defval} -> ${newDefVal} に変更しますか？`);
...
export const table = {  init, setup, setVal, setColor };
